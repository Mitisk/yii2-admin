...
class Breadcrumbs extends \yii\widgets\Breadcrumbs
{
     ...
     */
    public $options = ['class' => 'breadcrumbs flex items-center flex-wrap justify-start gap10'];
    /**
    ...
    public function run()
    {
        ...
        if ($this->homeLink === null) {
            $links[] = $this->renderItem([
                'label' => '<div class="text-tiny">' . Yii::t('yii', 'Home') . '</div>',
                'url' => Yii::$app->homeUrl,
            ], $this->itemTemplate);
        ...
        echo Html::tag($this->tag, implode('<li><i class="icon-chevron-right"></i></li>', $links), $this->options);
    }
    ...
    protected function renderItem($link, $template)
    {
        ...
        if (array_key_exists('label', $link)) {
            $label = $encodeLabel ? Html::encode($link['label']) : '<div class="text-tiny">' . $link['label'] . '</div>';
        } else {
