...
use kak\rbac\models\traits\BaseItem;
use Yii;
...
/**
 * Class AuthItem
 * @package Mitisk\Yii2Admin\models\rbac
...
class AuthItem extends \yii\base\Model
{
    ...
    public function search($params)
    {
        ...
        if (\yii\helpers\ArrayHelper::getValue(Yii::$app->request->get(), 'AuthItem.search')) {

            $search = strtolower(trim(\yii\helpers\ArrayHelper::getValue(Yii::$app->request->get(), 'AuthItem.search')));
            $items = array_filter($items, function ($item) use ($search) {
                return (empty($search) || strpos(strtolower($item->name), $search) !== false) && ( empty($desc) || strpos(strtolower($item->description), $search) !== false);
            });
        }
    ...
    public function rules()
    {
        return [
            ...
            [['description', 'data', 'search'], 'string'],
            [['type'], 'integer'],
            [['description', 'data', 'ruleName'], 'default'],
            [['rolesChildren','permissionsChildren', 'search'],'safe'],
